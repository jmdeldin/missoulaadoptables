...
class Loader
{
  ...
  function updateActive($c, $b)
  {
    ...
    mysql_query("UPDATE animals SET active = {$b} WHERE impound_num = '{$animal}';", $this->connection);
  }
  ...
  {
    for ($k = 2; $k < count($this->keys) - 3; $k++)
    {
      print "<br/>{$this->keys[$k]} - ";
